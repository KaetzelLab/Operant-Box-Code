...
target_port_list = [1, 2, 4, 5, 3]
Hole1_choices = [4, 3, 2]
Hole2_choices = [5, 4, 3, 1]
Hole3_choices = [5, 1, 2, 4]
Hole4_choices = [1, 2, 5, 4]
Hole5_choices = [2, 3, 4]
...
events = ['session_timer',
          ...
          'kill_port_lights', # timer to turn off port light
          'kill_reward',  # timer to turn off receptacle light
...
def sample_state(event):
    if event == 'entry':
        v.sp_target = 0
        print('sample_phase')
        set_timer('kill_port_lights', v.SP_state_dur)
...
def choice_state(event):
    if event == 'entry' and v.sp_target == 1:
        ...
        set_timer('kill_port_lights', v.CP_state_dur)
